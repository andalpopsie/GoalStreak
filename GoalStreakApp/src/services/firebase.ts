...
if (!validateEnvironmentConfig()) {
  console.warn('⚠️ Firebase configuration validation failed. Attempting to continue with available config...');
...
const firebaseConfig = {
  ...
  storageBucket: config.firebase.storageBucket || process.env.EXPO_PUBLIC_FIREBASE_STORAGE_BUCKET || '',
  messagingSenderId: config.firebase.messagingSenderId || process.env.EXPO_PUBLIC_FIREBASE_MESSAGING_SENDER_ID || '',
  appId: config.firebase.appId || process.env.EXPO_PUBLIC_FIREBASE_APP_ID || '',
  measurementId: config.firebase.measurementId || process.env.EXPO_PUBLIC_FIREBASE_MEASUREMENT_ID || '',
};
