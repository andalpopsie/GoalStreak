...
export const validateTimerForm = (form: TimerConfigForm): TimerValidationResult => {
  ...
  // Validate minutes
  if (typeof form.minutes !== 'number' || isNaN(form.minutes) || form.minutes < 0 || form.minutes > 59) {
    errors.push({
      field: 'general',
...
export const validateTimerState = (state: any): { isValid: boolean; errors: string[] } => {
  ...
  } else if (state.pausedTime > 24 * 60 * 60 * 1000) { // More than 24 hours
  ...
  if (typeof state.remainingTime !== 'number' || state.remainingTime < 0) {
    ...
    errors.push(`Timer remainingTime cannot exceed ${TIMER_CONSTANTS.MAX_DURATION_MINUTES / 60} hours`);
  }
  ...
  if (state.lastUpdate) {
    const lastUpdate = state.lastUpdate instanceof Date ? state.lastUpdate : new Date(state.lastUpdate);
