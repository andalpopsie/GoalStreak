...
const ERROR_CONFIG: Record<TimerError, {
  severity: TimerErrorSeverity;
  category: TimerErrorCategory;
  recoverable: boolean;
  retryable: boolean;
...
const USER_FRIENDLY_MESSAGES: Record<TimerError, string> = {
  ...
  [TimerError.TIMER_ALREADY_ACTIVE]: 'This habit already has an active timer. Stop it first or use the existing timer.',
  ...
  [TimerError.BACKGROUND_SYNC_FAILED]: 'Timer data will sync when you\'re back online. Your progress is saved locally.',
  [TimerError.NOTIFICATION_PERMISSION_DENIED]: 'Enable notifications in your device settings to get timer alerts.',
  [TimerError.STORAGE_QUOTA_EXCEEDED]: 'Your device is running low on storage. Some timer data may not be saved.',
  ...
  [TimerError.TIMER_PERSISTENCE_FAILED]: 'Unable to save timer progress. Your data may be lost if you close the app.',
  [TimerError.INVALID_TIMER_STATE]: 'Timer is in an invalid state. Try resetting the timer or restarting the app.'
};
...
export class TimerErrorHandler {
  ...
  private async retryStorageOperation(error: EnhancedTimerError): Promise<void> {
    try {
      ...
      if (storageInfo.availableSpace < 1024 * 1024) { // Less than 1MB
        Alert.alert(
          'Storage Full',
  ...
  validateTimerInput(input: {
    ...
    habitId?: string;
    userId?: string;
  }): { isValid: boolean; errors: TimerValidationError[] } {
    ...
    if (input.habitId !== undefined) {
      if (!input.habitId || typeof input.habitId !== 'string' || input.habitId.trim().length === 0) {
        errors.push({
          field: 'general',
  ...
  validateTimerState(state: Partial<TimerState>): { isValid: boolean; errors: string[] } {
    ...
    if (state.remainingTime !== undefined && (typeof state.remainingTime !== 'number' || state.remainingTime < 0)) {
