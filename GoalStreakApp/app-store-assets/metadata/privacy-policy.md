# Goalfer Privacy Policy

**Effective Date:** January 1, 2025  
**Last Updated:** July 31, 2026

## Introduction

Goalfer ("we," "our," or "us") is committed to protecting your privacy. This Privacy Policy explains how we collect, use, disclose, and safeguard your information when you use our mobile application ("Goalfer" or the "App"). This policy applies to all users of our iOS application.

By using Goalfer, you consent to the data practices described in this policy. If you do not agree with this policy, please do not use our App.

## Information We Collect

### Personal Information You Provide
- **Email Address**: Required for account creation, login, and communication
- **Display Name**: Your chosen name displayed to friends and in social features
- **Profile Picture**: Optional photo you upload for your profile
- **Account Preferences**: Settings, notification preferences, and privacy controls

### Habit and Usage Data
- **Habit Information**: Names, descriptions, categories, frequency settings, and target values
- **Completion Records**: Dates, times, and streak information for habit completions
- **Progress Data**: Analytics, charts, and achievement information
- **Custom Settings**: Personal categories, reminders, and app customizations

### Social Features Data
- **Friend Connections**: Email addresses of friends you invite or who invite you
- **Shared Activities**: Habit completions and progress you choose to share with friends
- **Social Interactions**: Reactions, comments, and engagement with friends' activities
- **Privacy Settings**: Your choices about what to share and with whom

### Automatically Collected Information
- **Device Information**: Device type, operating system version, unique device identifiers
- **App Usage Analytics**: Features used, time spent in app, navigation patterns
- **Performance Data**: Crash reports, error logs, and app performance metrics

### Subscription and Purchase Data
- **Purchase History**: Which Goalfer Pro subscription you hold (if any), purchase and renewal status, and entitlement information
- **How It Works**: All payments are processed by Apple through the App Store. We never receive or store your payment card details. Subscription status is validated and managed by our subscription provider, RevenueCat (see Third-Party Services below), which associates your subscription with an anonymous app user identifier
- **Purpose**: To unlock and maintain access to Goalfer Pro features and to understand subscription trends in aggregate

### Camera and Photo Library Access
- **Profile Pictures**: Photos you choose from your library or take with your camera
- **Usage**: Only accessed when you actively choose to update your profile picture
- **Storage**: Profile pictures are stored securely and only shared with your friends

## How We Use Your Information

### Core App Functionality
- Create and maintain your user account
- Enable habit tracking, streak counting, and progress analytics
- Provide personalized insights and recommendations
- Sync your data across devices
- Send habit reminders and motivational notifications
- Facilitate accountability groups and group progress tracking

### Social Features
- Connect you with friends who also use Goalfer
- Display your shared habit activities to friends (with your permission)
- Enable friend interactions like reactions and encouragement
- Facilitate accountability and motivation through social features

### App Improvement and Analytics
- Analyze app usage to improve features and user experience
- Monitor app performance and fix bugs
- Develop new features based on user behavior patterns
- Ensure app security and prevent fraud

### Communication
- Send important account and service notifications
- Provide customer support and respond to inquiries
- Notify you of app updates and new features
- Send habit-related reminders and motivational messages

## Data Sharing and Disclosure

### We DO NOT Sell Your Personal Data
Goalfer does not sell, rent, or trade your personal information to third parties for marketing purposes.

### Limited Sharing Scenarios
- **With Friends**: Only habit activities you explicitly choose to share
- **Service Providers**: Trusted third-party services that help us operate the app (Firebase, analytics providers)
- **Legal Requirements**: When required by law, court order, or to protect our rights
- **Business Transfers**: In the event of a merger, acquisition, or sale of assets (with notice to users)
- **Anonymous Data**: Aggregated, non-personally identifiable usage statistics for app improvement

### Social Feature Privacy
- You control what habit data is shared with friends
- Friends can only see activities you mark as "shareable"
- You can change privacy settings anytime
- You can remove friends or stop sharing at any time

## Data Security and Protection

### Security Measures
- All data encrypted in transit using industry-standard TLS encryption
- Data stored securely using Firebase's enterprise-grade security infrastructure
- Regular security audits and monitoring
- Secure authentication and access controls
- Protection against unauthorized access, alteration, disclosure, or destruction

### Data Retention
- Account data retained while your account is active
- Habit data retained to provide ongoing service and analytics
- You can delete your account and all associated data anytime
- Some data may be retained for legal compliance or security purposes
- Anonymous usage data may be retained indefinitely for app improvement

## Your Privacy Rights and Controls

### Access and Control
- **View Your Data**: Access all your personal data through the app
- **Update Information**: Modify your profile, habits, and settings anytime
- **Privacy Controls**: Choose what to share with friends and adjust privacy settings
- **Export Data**: Request a copy of your personal data
- **Delete Account**: Permanently delete your account and all associated data

### Communication Preferences
- **Notifications**: Control which notifications you receive
- **Marketing**: Opt out of promotional communications (we send very few)
- **Analytics**: Opt out of usage analytics in app settings

### Regional Privacy Rights
- **California Residents (CCPA)**: Right to know, delete, and opt-out of sale of personal information
- **EU Residents (GDPR)**: Rights to access, rectification, erasure, portability, and objection
- **Other Jurisdictions**: Rights as provided by applicable local privacy laws

## Children's Privacy

Goalfer is designed for users 13 years and older. We do not knowingly collect personal information from children under 13. If we discover we have collected information from a child under 13, we will delete it immediately. Parents who believe their child has provided information to us should contact us at hello@goalfer.app.

## International Data Transfers

Your information may be transferred to and processed in countries other than your own. We ensure appropriate safeguards are in place to protect your data in accordance with this privacy policy and applicable laws.

## Third-Party Services

### Firebase (Google)
- **Purpose**: Backend infrastructure, authentication, database, and analytics
- **Data Shared**: Account information, habit data, usage analytics
- **Privacy Policy**: https://policies.google.com/privacy

### RevenueCat (Subscription Management)
- **Purpose**: Manages Goalfer Pro subscriptions, validates App Store purchase receipts, and determines your access to Pro features
- **Data Shared**: An anonymous app user identifier and your subscription/purchase status. We do not send RevenueCat your payment details — those are handled solely by Apple
- **Privacy Policy**: https://www.revenuecat.com/privacy

### Apple App Store
- **Purpose**: Processes all subscription payments and renewals
- **Data Shared**: Payment and billing information is handled entirely by Apple under Apple's Privacy Policy; Goalfer never receives your payment card details
- **Privacy Policy**: https://www.apple.com/legal/privacy/

### Analytics Services
- **Purpose**: App performance monitoring and usage analytics
- **Data Shared**: Anonymous usage patterns and app performance data
- **Opt-Out**: Available in app settings

## Changes to This Privacy Policy

We may update this Privacy Policy periodically to reflect changes in our practices or applicable laws. We will:
- Post the updated policy in the app and on our website
- Notify users of significant changes through in-app notifications
- Update the "Last Updated" date at the top of this policy
- For material changes, provide 30 days notice before implementation

Continued use of the app after changes constitutes acceptance of the updated policy.

## Contact Information

### Privacy Questions and Requests
- **Email**: hello@goalfer.app
- **Subject Line**: Include "Privacy Request" for faster processing
- **Response Time**: We respond to privacy requests within 30 days

### General Support
- **Email**: hello@goalfer.app
- **Website**: www.goalfer.app

### Data Protection Officer
For EU residents or GDPR-related inquiries:
- **Email**: hello@goalfer.app

## Compliance and Certifications

Goalfer complies with:
- California Consumer Privacy Act (CCPA)
- General Data Protection Regulation (GDPR)
- Children's Online Privacy Protection Act (COPPA)
- Apple App Store Privacy Guidelines
- Google Play Privacy and Security Requirements

---

**Last Updated**: July 31, 2026  
**Version**: 1.2

*This privacy policy is effective as of the date listed above and applies to all users of the Goalfer mobile application.*