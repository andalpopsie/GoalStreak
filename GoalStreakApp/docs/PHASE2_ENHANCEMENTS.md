...
| Sep 25, 2026 | `updateUserProfile` in `useAuth.tsx` writes only to `users/{uid}` — it does not sync to `userProfiles/{uid}` (the social profile mirror). A display name change via Edit Profile is invisible in the activity feed, friend lists, and friend docs until the next activity write. Specific instance of issue #2 (stale names). Fix: extend `updateUserProfile` to also `setDoc` the same fields on `userProfiles/{uid}`. | `src/hooks/useAuth.tsx` → `updateUserProfile` | Important |
