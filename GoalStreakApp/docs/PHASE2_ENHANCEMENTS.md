...
### 3. Firestore Security Rules — Friend Collection ✅ Fixed — PR #75
...
| Sep 25, 2026 | `streaks` collection has `allow write: if isAuthenticated()` — any authenticated user can overwrite any streak doc. Should be split into create/update rules with `request.auth.uid == resource.data.userId` guard. Surfaced while fixing the friends collection rule. | `GoalStreakApp/firebase/firestore.rules` | Important |
| Sep 25, 2026 | `timerStates` collection has `allow read, write: if isAuthenticated()` — no owner validation. Low risk (transient data) but inconsistent with the rest of the security model. Should restrict to the owning user. | `GoalStreakApp/firebase/firestore.rules` | Low |
