...
| Sep 25, 2026 | Legacy duplicate `friends` docs (auto-ID) from before the compound-ID fix still exist in Firestore. A one-time cleanup script should query each user's friends by `userId`+`friendId`, keep the most recent doc, and delete extras. No user-visible impact but wastes reads/storage. | `friends` Firestore collection | Important |
